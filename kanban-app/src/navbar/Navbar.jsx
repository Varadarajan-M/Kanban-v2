...
const Navbar = () => {
  ...
  const { saveState, saveChanges, projectDetails, projectList, removeProject } =
    useProjectData();
  ...
  return (
    <div className='navbar__wrapper'>
      <nav className='navbar'>
        ...
        <span className='username hide-md'>
          {username ?? 'User'} / Kanban App
        </span>
        ...
        <div className='nav__buttons'>
          ...
          <MenuContainer className={haveNoProjects ? 'ml-2' : ''}>
            ...
            {avatarClicked ? (
              <Menu
                style={{ margin: '16px 0px 0px -34px' }}
                onBlur={() => setTimeout(closeAvatarMenu, 100)}
              >
                <MenuItem
                  onClick={() => {
                    clearAuthState();
