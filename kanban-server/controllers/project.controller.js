...
exports.get = async (req, res) => {
	...
	if (projectData.ok) {
		...
		const CANNOT_GET_USERS_ERR = throwError('Cannot get Project details', 400);
		sendError(res, CANNOT_GET_USERS_ERR);
	}
};
...
exports.getSharedUsers = async (req, res) => {
	const resp = await ProjectService.getSharedUsers(req.params?.id ?? -1, req?.user?.userID);
	if (resp.ok) {
		sendSuccessPayload(res, resp.users, 200);
	} else {
		const CANNOT_GET_USERS_ERR = throwError('Cannot get user details', 400);
		sendError(res, CANNOT_GET_USERS_ERR);
	}
};
